...
class Room:
    ...
    def changed(self, data: client.Datas) -> int:
        temp1: list[dict[str, str]] = [{"name": card.name, "history": card.history,"covered": card.covered } for card in self.items]
        temp2: list[dict[str, str]] = [{"name": card.name, "history": card.history, "covered": card.covered} for card in data.items]
        if temp1 != temp2:
            return 1
    ...
    def change(self, online: client.Client):
        online.send_data(self.name, [{"name": card.name, "history": card.history, "covered": card.covered} for card in self.items], self.players)
